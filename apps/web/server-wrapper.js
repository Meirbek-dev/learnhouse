...
const knownPublicVars = [
  ...
  'NEXT_PUBLIC_PLATFORM_TOP_DOMAIN',
  'NEXT_PUBLIC_PLATFORM_MULTI_ORG',
  'NEXT_PUBLIC_PLATFORM_DEFAULT_ORG',
  'NEXT_PUBLIC_PLATFORM_HTTPS',
...
knownPublicVars.forEach((key) => {
  if (env[key]) {
    ...
    switch (key) {
      case 'NEXT_PUBLIC_PLATFORM_API_URL':
        // Default to the backend port commonly used by the API service
        runtimeConfig[key] = 'http://localhost:9000/api/v1/';
        break;
      case 'NEXT_PUBLIC_PLATFORM_BACKEND_URL':
        // Backend root URL default (include port so fetches target the API service)
        runtimeConfig[key] = 'http://localhost:9000/';
        break;
      case 'NEXT_PUBLIC_PLATFORM_DOMAIN':
        runtimeConfig[key] = 'localhost';
        break;
      case 'NEXT_PUBLIC_PLATFORM_TOP_DOMAIN':
        runtimeConfig[key] = 'localhost';
        break;
      case 'NEXT_PUBLIC_PLATFORM_MULTI_ORG':
        runtimeConfig[key] = 'false';
        break;
      case 'NEXT_PUBLIC_PLATFORM_DEFAULT_ORG':
        runtimeConfig[key] = 'default';
        break;
      case 'NEXT_PUBLIC_PLATFORM_HTTPS':
        runtimeConfig[key] = 'false';
        break;
    }
...
try {
  ...
  fs.writeFileSync(scriptPath, `window.__RUNTIME_CONFIG__ = ${JSON.stringify(runtimeConfig)};`, 'utf8');
} catch {
