...
export const GamificationProfileProvider: React.FC<{ children: React.ReactNode; orgId: number }> = ({ children, orgId }) => {
  const { data: session } = useSession();
  const [profile, setProfile] = useState<GamificationProfile | null>(null);
  const inflight = useRef<Promise<any> | null>(null);

  const refresh = useCallback(async (oid: number = orgId) => {
  ...
  const optimisticAward = useCallback(async (oid: number, amount: number, source: string, extras: Partial<XPAwardRequest> = {}) => {
    if (!(session?.tokens?.access_token && oid)) return;
    setProfile((prev) => {
      if (!prev) return prev;
