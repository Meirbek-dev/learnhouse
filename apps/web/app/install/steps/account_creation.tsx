...
function AccountCreation() {
  ...
  const validate = (values: Omit<AccountCreationFormValues, 'org_slug'>) => {
    const errors: Partial<Record<keyof Omit<AccountCreationFormValues, 'org_slug'>, string>> = {} // More specific type
  ...
  const formik = useFormik<Omit<AccountCreationFormValues, 'org_slug'>>({ // Use the specific type
    initialValues: {
      email: '',
      password: '',
      confirmPassword: '',
   ...
   if (fetchError) return <div>{generalT('error')}: {typeof fetchError === 'object' && fetchError !== null && 'message' in fetchError ? String(fetchError.message) : String(fetchError)}</div>; // Safely access error message
    ...
    <div>
      ...
      <FormLayout onSubmit={formik.handleSubmit}>
        <FormField name="email">
          ...
          <FormLabelAndMessage label={t('emailLabel')} message={formik.touched.email && formik.errors.email ? formik.errors.email : undefined} />
          ...
          {/* Optionally add error display specific to the field */}
           {formik.touched.email && formik.errors.email && <span id="email-error" className="text-red-600 text-sm">{formik.errors.email}</span>}
        </FormField>

        <FormField name="password">
          <FormLabelAndMessage
            label={t('passwordLabel')} // Use translation key
            message={formik.touched.password && formik.errors.password ? formik.errors.password : undefined}
          />
          <Form.Control asChild>
            <Input
           ...
           {formik.touched.password && formik.errors.password && <span id="password-error" className="text-red-600 text-sm">{formik.errors.password}</span>}
        </FormField>

        <FormField name="confirmPassword">
          <FormLabelAndMessage
            label={t('confirmPasswordLabel')} // Use translation key
            message={formik.touched.confirmPassword && formik.errors.confirmPassword ? formik.errors.confirmPassword : undefined}
          />
          <Form.Control asChild>
            <Input
           ...
           {formik.touched.confirmPassword && formik.errors.confirmPassword && <span id="confirmPassword-error" className="text-red-600 text-sm">{formik.errors.confirmPassword}</span>}
        ...
        <FormField name="username">
          <FormLabelAndMessage
            ...
            message={formik.touched.username && formik.errors.username ? formik.errors.username : undefined}
          />
          <Form.Control asChild>
            <Input
           ...
           {formik.touched.username && formik.errors.username && <span id="username-error" className="text-red-600 text-sm">{formik.errors.username}</span>}
        </FormField>
