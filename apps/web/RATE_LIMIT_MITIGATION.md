...
The upstream Nginx proxy at `cs-mooc.tou.edu.kz` applies request rate limits that can trigger HTTP 429 errors during initial page loads when many JS chunks are fetched in parallel.
...
2. Throttle network to "Slow 3G" and reload. Watch Network tab – chunks should come from ServiceWorker after first load.
...
1. **Reduce chunk count** – Enable `experimental.optimizePackageImports` in `next.config.ts` and audit barrel exports.
2. **External CDN** – Serve `/_next/static/*` from Cloudflare/Vercel Edge to bypass Nginx limits entirely.
...
5. **Rate-limit awareness in API calls** – Add retry logic to remaining services (courses, blocks, etc.) using `fetchWithRetry`.
