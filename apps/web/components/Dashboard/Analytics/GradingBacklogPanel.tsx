    ...
    <Card className="border-amber-200 bg-linear-to-br from-amber-50 via-white to-white shadow-sm">
      ...
      <CardContent>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {gradingAlerts.length ? gradingAlerts.map((alert) => <Badge key={alert.id} variant={alert.severity === 'critical' ? 'destructive' : 'warning'}>{alert.title}</Badge>) : <Badge variant="outline">{t('gradingBacklog.noAlert')}</Badge>}
        </div>
      </CardContent>
    </Card>
  );
