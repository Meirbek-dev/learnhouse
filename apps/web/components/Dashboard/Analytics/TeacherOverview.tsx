    ...
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-6 px-4 py-6 md:px-6 xl:px-8">
      <section className="overflow-hidden rounded-[2rem] border border-slate-200/80 bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.18),_transparent_32%),radial-gradient(circle_at_top_right,_rgba(14,165,233,0.16),_transparent_26%),linear-gradient(180deg,_#ffffff,_#f8fafc_56%,_#ecfdf5)] p-6 shadow-sm md:p-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">{t('overview.label')}</div>
            <h1 className="mt-3 max-w-3xl text-3xl font-semibold tracking-tight text-slate-950 md:text-4xl">{t('overview.heading')}</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600 md:text-base">
              {t('overview.description')}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <AnalyticsExportButton href={getAnalyticsExportUrl(orgId, 'at-risk', query)} label={t('overview.exportAtRisk')} />
            <AnalyticsExportButton href={getAnalyticsExportUrl(orgId, 'grading-backlog', query)} label={t('overview.exportGradingBacklog')} />
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
        <AnalyticsMultiSeriesTrendChart title={t('overview.trendTitle')} description={t('overview.trendDescription')} data={trendData} />
        ...
        <Card className="border-slate-200 bg-white/90 shadow-sm">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-2 text-lg font-semibold text-slate-900">{new Date(data.generated_at).toLocaleString(locale)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="text-xs uppercase tracking-[0.18em] text-slate-500">{t('overview.labelScopedCourses')}</div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-2 text-lg font-semibold text-slate-900">{data.scope.cohort_ids.length || t('overview.cohortsAll')}</div>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {data.alerts.length ? data.alerts.map((alert) => (
            <div key={alert.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="mb-2 flex items-center gap-2">
                <Badge variant={alert.severity === 'critical' ? 'destructive' : alert.severity === 'warning' ? 'warning' : 'outline'}>{getAnalyticsSeverityLabel(t, alert.severity)}</Badge>
                <span className="text-xs uppercase tracking-[0.18em] text-slate-500">{getAnalyticsAlertTypeLabel(t, alert.type)}</span>
              </div>
        ...
        <div>
          ...
          {courseRows.length > 8 && (
            <p className="mt-2 text-sm text-slate-500">
              ...
              <Link href={`/orgs/${orgslug}/dash/analytics/courses`} className="text-blue-600 hover:underline">{t('overview.viewAllCourses')}</Link>
            </p>
          )}
        </div>
        <div>
          ...
          {assessmentRows.length > 8 && (
            <p className="mt-2 text-sm text-slate-500">
              ...
              <Link href={`/orgs/${orgslug}/dash/analytics/assessments`} className="text-blue-600 hover:underline">{t('overview.viewAllAssessments')}</Link>
            </p>
          )}
        </div>
      </div>

      <AtRiskLearnersTable rows={data.at_risk_preview} title={t('overview.watchlistTitle')} description={t('overview.watchlistDescription')} />
    </div>
  );
}
