...
export default function CourseCreationWizard() {
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        if (
          !result.success ||
          !createdCourse ||
          !("course_uuid" in createdCourse)
        ) {
          const detail =
            createdCourse &&
            typeof createdCourse === "object" &&
            "detail" in createdCourse
              ? createdCourse.detail
              : undefined;
          throw new Error(
            (typeof detail === "string" ? detail : undefined) ||
              t("errors.creationFailed"),
          );
        }
        ...
        router.replace(
          buildCourseWorkspacePath(createdCourse.course_uuid, "curriculum"),
        );
        router.refresh();
      } catch (error: unknown) {
        toast.error(
  ...
  useEffect(() => {
    ...
    if (
      templateParam === "outline" ||
      templateParam === "starter" ||
      templateParam === "blank"
    ) {
  ...
  const createOutlineFromSource = async (createdCourse: any) => {
    ...
    const chapters = Array.isArray(sourceMetadata?.chapters)
      ? sourceMetadata.chapters
      : [];
  ...
  const summaryContent = (
    <div className="text-muted-foreground space-y-4 text-sm">
      ...
      <div>
        ...
        <div className="mt-1">
          {isPublic
            ? t("visibility.public.summary")
            : t("visibility.private.summary")}
        </div>
      </div>
      <div>
      ...
      {template === "outline" && sourceCourseUuid ? (
        <div>
          <div className="text-muted-foreground">
            {t("summary.sourceCourse")}
          </div>
          <div className="mt-1">
            {selectedSourceName || t("summary.selectedOutlineCourse")}
          </div>
        </div>
      ) : null}
    </div>
            ...
            <div className="space-y-5">
              ...
              <fieldset className="space-y-3">
                ...
                <RadioGroup
                  ...
                  onValueChange={(val) =>
                    form.setFieldValue("public", val === "public")
                  }
                ...
                >
                  {[
                    ...
                    <CourseChoiceCard
                      ...
                      value={option.value}
                      checked={
                        (isPublic ? "public" : "private") === option.value
                      }
                      title={option.title}
                ...
                <CollapsibleContent className="bg-card mt-4 space-y-5 rounded-xl border p-4">
                  <RadioGroup
                    ...
                    onValueChange={(val) =>
                      form.setFieldValue(
                        "template",
                        val as CourseWizardValues["template"],
                      )
                    }
                    ...
                    {[
                      ...
                      <CourseChoiceCard
                        ...
                        onSelect={(value) =>
                          form.setFieldValue(
                            "template",
                            value as CourseWizardValues["template"],
                          )
                        }
                      />
                    ))}
                          ...
                          {sourceOptions.map((course) => (
                            <button
                              ...
                              className={cn(
                                ...
                                sourceCourseUuid === course.cleanUuid &&
