...
function DropdownMenuTrigger({ render, children, ...props }: DropdownMenuTriggerProps) {
  // The underlying primitive supports either a `render` function or
  // regular children. We expose a simple `render` prop for compatibility
  // with the previous API, but we ultimately just render whatever node is
  // passed in as `render` or `children` as the trigger content.
  if (render) {
    return (
