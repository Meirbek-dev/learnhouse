...
down_revision: Union[str, None] = 'add4ea7479ad'
...
def upgrade() -> None:
    ...
    # 4. Add composite index on resource_authors for ownership checks
    print("Creating index: idx_resource_authors_resource_user...")
    conn.execute(text("""
    ...
    # 5. Add index on resource_authors for user-based queries
    print("Creating index: idx_resource_authors_user_id...")
    op.create_index(
        'idx_resource_authors_user_id',
        'resource_authors',
        ['user_id'],
        unique=False
    )
    
    # 6. Add composite index for permission audit log queries
    print("Creating index: idx_audit_log_user_resource...")
    conn.execute(text("""
...
def downgrade() -> None:
    ...
    op.drop_index('idx_audit_log_user_resource', table_name='permission_audit_log', if_exists=True)
    op.drop_index('idx_resource_authors_user_id', table_name='resource_authors', if_exists=True)
    op.drop_index('idx_resource_authors_resource_user', table_name='resource_authors', if_exists=True)
    ...
    op.drop_index('idx_permissions_resource_action', table_name='permissions', if_exists=True)
