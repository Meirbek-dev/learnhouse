...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['parent_role_id'], ['roles_new.id'], ondelete='SET NULL'),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['permission_id'], ['permissions.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['granted_by'], ['user.id'], ondelete='SET NULL'),
        sa.UniqueConstraint('user_id', 'resource_type', 'resource_id', 'permission_id', name='uq_resource_permission'),
    )
    op.create_index('ix_resource_permissions_user_resource', 'resource_permissions', ['user_id', 'resource_type', 'resource_id'])
...
def downgrade() -> None:
    ...
    op.drop_index('ix_resource_permissions_user_resource', table_name='resource_permissions')
...
def _seed_default_permissions() -> None:
    ...
    permissions = [
        ...
        ('organization:read:own', 'organization', 'read', 'own', 'Read own organization'),
        ('organization:update:own', 'organization', 'update', 'own', 'Update own organization'),
        ('organization:manage:own', 'organization', 'manage', 'own', 'Manage own organization settings'),
        ('organization:delete:own', 'organization', 'delete', 'own', 'Delete own organization'),
        # Course permissions
        ('course:create:org', 'course', 'create', 'org', 'Create courses in organization'),
        ...
        ('course:update:org', 'course', 'update', 'org', 'Update courses in organization'),
        ('course:delete:own', 'course', 'delete', 'own', 'Delete own courses'),
        ('course:delete:org', 'course', 'delete', 'org', 'Delete courses in organization'),
        ('course:manage:own', 'course', 'manage', 'own', 'Manage own course settings'),
        ('course:manage:all', 'course', 'manage', 'all', 'Manage all courses'),
        # Chapter permissions
        ('chapter:create:own', 'chapter', 'create', 'own', 'Create chapters in own courses'),
        ('chapter:create:org', 'chapter', 'create', 'org', 'Create chapters in organization'),
        ('chapter:read:all', 'chapter', 'read', 'all', 'Read chapters'),
        ('chapter:update:own', 'chapter', 'update', 'own', 'Update own chapters'),
        ('chapter:update:org', 'chapter', 'update', 'org', 'Update chapters in organization'),
        ('chapter:delete:own', 'chapter', 'delete', 'own', 'Delete own chapters'),
        ('chapter:delete:org', 'chapter', 'delete', 'org', 'Delete chapters in organization'),
        # Activity permissions
        ('activity:create:own', 'activity', 'create', 'own', 'Create activities in own courses'),
        ('activity:create:org', 'activity', 'create', 'org', 'Create activities in organization'),
        ('activity:read:all', 'activity', 'read', 'all', 'Read activities'),
        ('activity:update:own', 'activity', 'update', 'own', 'Update own activities'),
        ('activity:update:org', 'activity', 'update', 'org', 'Update activities in organization'),
        ('activity:delete:own', 'activity', 'delete', 'own', 'Delete own activities'),
        ('activity:delete:org', 'activity', 'delete', 'org', 'Delete activities in organization'),
        # User permissions
        ...
        ('usergroup:create:org', 'usergroup', 'create', 'org', 'Create usergroups in organization'),
        ('usergroup:read:org', 'usergroup', 'read', 'org', 'Read usergroups in organization'),
        ('usergroup:update:org', 'usergroup', 'update', 'org', 'Update usergroups in organization'),
        ('usergroup:delete:org', 'usergroup', 'delete', 'org', 'Delete usergroups in organization'),
        # Collection permissions
        ('collection:create:org', 'collection', 'create', 'org', 'Create collections in organization'),
        ('collection:read:all', 'collection', 'read', 'all', 'Read public collections'),
        ('collection:update:own', 'collection', 'update', 'own', 'Update own collections'),
        ('collection:update:org', 'collection', 'update', 'org', 'Update collections in organization'),
        ('collection:delete:own', 'collection', 'delete', 'own', 'Delete own collections'),
        ('collection:delete:org', 'collection', 'delete', 'org', 'Delete collections in organization'),
        # Role permissions
        ...
        ('certificate:create:own', 'certificate', 'create', 'own', 'Create certificates for own courses'),
        ('certificate:read:all', 'certificate', 'read', 'all', 'Read certificates'),
        # Analytics permissions
        ('analytics:read:own', 'analytics', 'read', 'own', 'Read own analytics'),
        ('analytics:read:org', 'analytics', 'read', 'org', 'Read organization analytics'),
        # Assignment/Quiz permissions
        ('assignment:grade:own', 'assignment', 'grade', 'own', 'Grade assignments in own courses'),
        ...
        ('discussion:moderate:org', 'discussion', 'moderate', 'org', 'Moderate discussions in organization'),
    ]
...
def _seed_default_roles() -> None:
    ...
    roles = [
        ('super-admin', 'Super Admin', 'Platform-wide administrator with full access', True, 100, None),
        ('org-admin', 'Organization Admin', 'Full control over organization', True, 90, None),
        ('maintainer', 'Maintainer', 'Content management and course administration', True, 70, None),
    ...
    # Insert roles
    for slug, name, description, is_system, priority, parent_slug in roles:
        conn.execute(
            sa.text("""
    ...
    # Get role IDs
    result = conn.execute(sa.text("SELECT id, slug FROM roles_new WHERE org_id IS NULL"))
    ...
    role_permissions = {
        ...
        'org-admin': [
            p for p in perm_ids.keys()
            if any(x in p for x in ['organization:', 'course:', 'chapter:', 'activity:',
                                     'user:', 'usergroup:', 'collection:', 'role:',
                                     'analytics:', 'file:', 'api_token:'])
        ],
        'maintainer': [
            p for p in perm_ids.keys()
            if any(x in p for x in ['course:', 'chapter:', 'activity:', 'collection:',
                                     'user:read', 'usergroup:read', 'analytics:read'])
        ],
        'instructor': [
            'course:create:org', 'course:read:all', 'course:update:own', 'course:delete:own',
            'course:manage:own', 'chapter:create:own', 'chapter:read:all', 'chapter:update:own',
            'chapter:delete:own', 'activity:create:own', 'activity:read:all', 'activity:update:own',
            'activity:delete:own', 'assignment:grade:own', 'quiz:grade:own', 'certificate:create:own',
            'analytics:read:own', 'user:read:own',
        ],
        'moderator': [
            'course:read:all', 'user:read:org', 'discussion:moderate:org',
        ],
        'user': [
            'course:read:all', 'user:read:own', 'user:update:own', 'assignment:submit:all',
            'quiz:submit:all', 'analytics:read:own', 'collection:read:all', 'certificate:read:all',
        ],
    }
