...
@pytest.mark.asyncio
async def test_get_user_session_includes_access_expiry_and_session_version(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr('src.services.users.users.PermissionChecker', lambda _db_session: checker)
    monkeypatch.setattr('src.security.auth.get_access_token_from_request', lambda _request, _token=None: 'token')
    monkeypatch.setattr(
        'src.security.auth.decode_access_token',
        lambda _token: SimpleNamespace(expires_at=1_700_000_000, issued_at=1_699_999_000),
    )
