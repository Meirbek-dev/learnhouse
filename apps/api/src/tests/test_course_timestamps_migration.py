...
def test_course_creation_update_columns_are_timestamptz():
    ...
    assert c is not None and u is not None, "course creation_date/update_date columns not present"

    def _contains(col_type, substr: str) -> bool:
        return substr.upper() in str(col_type).upper()

    assert _contains(c.get("type"), "TIMESTAMP"), f"course.creation_date is not TIMESTAMP: {c.get('type')}"
    assert _contains(u.get("type"), "TIMESTAMP"), f"course.update_date is not TIMESTAMP: {u.get('type')}"
