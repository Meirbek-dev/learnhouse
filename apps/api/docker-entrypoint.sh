...
if [ -n "$PLATFORM_SQL_CONNECTION_STRING" ]; then
    ...
    DB_PORT=$(echo "$PLATFORM_SQL_CONNECTION_STRING" | sed -n 's/.*@\([^:]*\):\([0-9]*\)\/.*/\2/p')
...
if [ -n "$PLATFORM_REDIS_CONNECTION_STRING" ]; then
    # Extract host and port from redis://host:port/db or redis://host:port
    REDIS_HOST=$(echo "$PLATFORM_REDIS_CONNECTION_STRING" | sed -n 's|redis://\([^:/]*\):\([0-9]*\).*|\1|p')
    REDIS_PORT=$(echo "$PLATFORM_REDIS_CONNECTION_STRING" | sed -n 's|redis://\([^:/]*\):\([0-9]*\).*|\2|p')
    ...
    if [ -z "$REDIS_HOST" ]; then
        ...
        REDIS_HOST=$(echo "$PLATFORM_REDIS_CONNECTION_STRING" | sed -n 's|redis://\([^:/]*\).*|\1|p')
    fi
...
PORT=${PLATFORM_PORT:-9000}
HOST=${HOSTNAME:-0.0.0.0}

echo "Starting Ashyq Bilim backend on ${HOST}:${PORT}..."
