...
with engine.connect() as conn:
    ...
    try:
        conn.execute(
            text("ALTER TABLE permissions ALTER COLUMN resource_type TYPE VARCHAR")
        )
        ...
        conn.execute(
            text(
                "ALTER TABLE resource_permissions ALTER COLUMN resource_type TYPE VARCHAR"
            )
        )
        conn.commit()
    ...
    result3 = conn.execute(
        text("UPDATE permissions SET scope = LOWER(scope) WHERE scope != LOWER(scope)")
    )
    ...
    result = conn.execute(
        text(
            "SELECT COUNT(*) FROM permissions WHERE resource_type != LOWER(resource_type) OR action != LOWER(action) OR scope != LOWER(scope)"
        )
    )
